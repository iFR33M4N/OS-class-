...
{
	slob_t *prev, *best_prev, *cur, *aligned = NULL, *best_fit = NULL;
	int delta = 0, units = SLOB_UNITS(size);
	unsigned long frag = -1UL;
	...
	for (prev = NULL, cur = sp->free; ; prev = cur, cur = slob_next(cur)) {
		...
		if (avail >= units + delta && frag > avail - units) {/* room enough? */
			frag = avail-units;
			best_fit = cur;
			best_prev = prev;
			if ( avail == units)
	...
	if (best_fit) {
		...
		if (avail == units) { /* exact fit? unlink. */
			...
			if (best_prev)
				set_slob(prev, slob_units(best_prev), best_fit + units);
			else
				sp->free = best_fit + units;
...
{
	struct slob_page *sp, *best_sp;
	struct list_head *prev, *best_prev;
	struct list_head *slob_list;
	slob_t *b = NULL;
	unsigned long flags, best_size = -1UL;
	...
	list_for_each_entry(sp, slob_list, list) {
		...
		if (best_size < sp->units)
			continue;
			
		best_sp = sp;
			
		best_size = sp->units;
		best_prev = sp->list.prev;
		
		if (best_size - SLOB_UNITS(size) != 0)
			continue;
		else {
